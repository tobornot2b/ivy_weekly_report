...
SPEED_DATE_N = '2023-02-01' # 동복 PEAK 시작일
...
def data_preprocess(season: str, df: pd.DataFrame) -> pd.DataFrame:
    if season[-1] != 'S': # 동복
        ...
        # 순서대로 조립
        # 남자
        df_temp = df1.query('성별 == "남" and 복종 == "J"').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        df_temp = df1.query('성별 == "남" and 복종 == "Y"').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        df_temp = df1.query('성별 == "남" and 복종 == ["P", "Q"]').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        # 여자
        df_temp = df1.query('성별 == "여" and 복종 == "J"').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        df_temp = df1.query('성별 == "여" and 복종 == "B"').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        df_temp = df1.query('성별 == "여" and 복종 == ["P", "Q"]').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        df_temp = df1.query('성별 == "여" and 복종 == ["S", "R"]').copy()
        df_common = pd.concat([df_common, df_temp], axis=0)

        # 공통
        df_temp = df1.query("복종 == 'V'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['V']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '공통', '30', 'V', '*']
        df_common.loc[11] = df_temp.loc[-1].copy() # 맨 마지막에 추가. 인덱스명은 의미 없는 구분용.

        df_temp = df1.query("복종 == 'K'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['K']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '공통', '32', 'K', '*']
        df_common.loc[12] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'G'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['G']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '공통', '34', 'G', '*']
        df_common.loc[13] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'N'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['N']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '공통', '60', 'N', 'S']
        df_common.loc[14] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'C'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['C']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '공통', '40', 'C', '*']
        df_common.loc[15] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'W'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['W']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['체육복', '공통', '42', 'W', '*']
        df_common.loc[16] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'F'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['F']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['체육복', '공통', '44', 'F', '*']
        df_common.loc[17] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'H'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['H']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['체육복', '공통', '70', 'H', 'S']
        df_common.loc[18] = df_temp.loc[-1].copy()

        # 자켓기준
        df_temp = df1.query("복종 == 'J'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['J']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '자켓기준', '', '', 'S']
        df_common.loc[19] = df_temp.loc[-1].copy()

        # 하의기준
        df_temp = df1.query("복종 == ['P', 'Q', 'S', 'R']").copy()
        df_temp.loc[-1] = df1.query("복종 == ['P', 'Q', 'S', 'R']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['학생복', '하의기준', '', '', 'H']
        df_common.loc[20] = df_temp.loc[-1].copy()

        # st.dataframe(df_common)

        df_prod_report = df_common.reset_index(drop=True)
        ...
        df_prod_report['출고율'].fillna(0, inplace=True) # 0으로 나누는 경우가 있어서 0으로 채움
        
        df_prod_report2 = df_prod_report[['성별', '복종', '홀드', '본사', '원단', '타입', '완료', '출고율']].copy()

        # st.dataframe(df_prod_report2)
        ...
        df_temp = df1.query("복종 == 'W'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['W']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['체육복', '공통', '21', 'W', '*']
        df_prod_report.loc['W'] = df_temp.loc[-1].copy()

        df_temp = df1.query("복종 == 'F'").copy()
        df_temp.loc[-1] = df1.query("복종 == ['F']").sum(axis=0)
        df_temp.loc[-1, '제품':'상하의'] = ['체육복', '공통', '22', 'F', '*']
        df_prod_report.loc['F'] = df_temp.loc[-1].copy()
...
if choosen_season_prod[-1] == 'N':
    prod_quota = [str(int(choosen_season_prod[:2])-1) + 'F', str(int(choosen_season_prod[:2])-1) + 'W', choosen_season_prod] # 이번 시즌 쿼터
elif choosen_season_prod[-1] == 'S':
    prod_quota = [choosen_season_prod, choosen_season_prod, choosen_season_prod]
...
if choosen_season_prod[-1] == 'S': # 하복
    ivy_type_qty = df_prod.query("성별 == '하의기준'").iat[-1, 5] # '타입', 아이비 타입량
    ivy_product = df_prod.query("성별 == '하의기준'").iat[-1, 6] # '완료', 아이비 생산량
else: # 동복
    ivy_type_qty = df_prod.query("성별 == '자켓기준'").iat[-1, 5] # '타입', 아이비 타입량
    ivy_product = df_prod.query("성별 == '자켓기준'").iat[-1, 6] # '완료', 아이비 생산량
    ...
    if example == 1:
        ...
        with st.sidebar:
            selected = option_menu(
                ...
                options=['진행현황', '상세현황', '생산시간', '체크리스트'],  # required
...
if selected == "진행현황":
    # mod.draw_plan(mod.plan_data, '생산팀') # MASTER PLAN
